...
We'll be working in the kagent dashboard, so make sure you start the port forward:

```bash
kubectl port-forward -n kagent svc/kagent-app 8001:80
```

If you open your browser to [http://localhost:8001](http://localhost:8001) you should see the kagent dashboard as shown in the figure below.
...
![Kagent with tools](/images/kagent-new.png)
...
![configuring tools](/images/kagent-config.png)
...
![agent response](/images/kagent-chat-1.png)
