...
For Anthropic's Claude models, kagent automatically configures the appropriate model capabilities, including vision support for Claude-3 models. You can override these settings using the `modelInfo` field if needed. For more information about model capabilities and configuration, see [Model Information](/docs/concepts/model-info).
