...
1. Review the `kmcp.yaml` configuration of your MCP project. The default `kmcp.yaml` file includes a `secrets` section that defines multiple environments, such as `staging`, `production`, and `local`. Note that all environments are currently disabled. 
   ```sh
