...
🚀 Next steps:
   ...
   5. Deploy the agent to your local cluster
      kagent deploy myagent --api-key-secret <secret-name>
      Or use --api-key for convenience: kagent deploy myagent --api-key <api-key>
      Support for using a credential file is coming soon
```
...
```shell
kagent deploy myagent --api-key $OPENAI_API_KEY --dry-run
```
...
```shell
kagent deploy myagent --api-key $OPENAI_API_KEY
```
