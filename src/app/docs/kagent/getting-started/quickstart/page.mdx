...
1. From the kagent UI landing page, find your `kagent/my-first-k8s-agent` agent. You might have to refresh the page.
   ...
   ![kagent agent list](/images/kagent-first-k8s-agent.png "Your first agent")
...
3. Click around to explore the UI some more.
   ...
   * The **Agent Details** panel shows the tools and memory database sources that the agent uses to respond to your messages.
...
Interact with kagent in your terminal. Note that to use the CLI, you must have installed kagent with `kagent install` command.

1. Run *kagent* to start an interactive REPL environment with kagent. You can enter `help` to view the available commands.
   ...
   Commands:
   Commands:
     bug-report      Generate a bug report with system information
...
3. To start a new agent, run the `run chat` command:
...
4. Enter a task. Then, the agent starts running and you'll see the conversation in the CLI.
