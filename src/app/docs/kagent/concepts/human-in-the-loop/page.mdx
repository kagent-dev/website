...
Tool approval is configured in the Agent CR (not in the UI). Add `requireApproval` to your Agent's tool specification. The values must be a subset of `toolNames`:
...
In this example, `k8s_get_resources` and `k8s_describe_resource` execute immediately, but `k8s_delete_resource` and `k8s_apply_manifest` will pause for user approval.
