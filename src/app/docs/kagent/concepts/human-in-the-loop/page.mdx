...
In this example, `k8s_get_resources` executes immediately, but `k8s_apply_manifest` and `k8s_delete_resource` will pause for user approval.
...
- **Confirmation-gated execution** — Approved calls are re-invoked by feeding the confirmation response back through the ADK runner, which re-enters the flow at the point where the tool was blocked. The `before_tool_callback` acts as the gate, allowing execution to proceed on approval.
