...
1. Save your current kagent version and the kagent version that you want to roll back to.
   ```bash
   export CURRENT_VERSION=<current-version>
   export ROLLBACK_VERSION=<target-version>
   ```
...
4. Reset the core track. Use the `CURRENT_VERSION` source and `goto` the highest migration sequence number in the `go/core/pkg/migrations/core/` directory of `ROLLBACK_VERSION`. The `github://` source references the migration files directly from the release tag without a local checkout.
   ```bash
   migrate \
