...
Kagent uses PostgreSQL as the database backend. By default, a bundled PostgreSQL instance with pgvector is deployed when you install kagent. This bundled instance works out of the box with no external prerequisites.
...
To use an external PostgreSQL instance instead of the bundled one, set `database.postgres.url` or `database.postgres.urlFile`.
