...
Previously, the kagent CRD Helm chart installed the CRDs for both kagent and kmcp. Now, the kmcp CRDs are installed in a separate Helm chart.
...
The ToolServer API is completely removed from kagent. Instead, use [kmcp APIs](https://kagent.dev/docs/kmcp) to create and manage tools.
...
<div id="kmcp-tab">
  kmcp example:
  * The MCPServer includes the Grafana deployment details, with `stdio` as the transport type.
