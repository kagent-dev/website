...
- A PostgreSQL instance with pgvector is now bundled in the Helm chart by default. This bundled PostgreSQL is deployed automatically when both `database.postgres.url` and `database.postgres.urlFile` are empty.
