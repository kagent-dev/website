...
3. Create a BYO Agent resource.
   ...
   For private container registries, add `imagePullSecrets` to reference a Secret that contains credentials for pulling the agent image. Create the secret with `kubectl create secret docker-registry my-registry-secret --docker-server=<registry> --docker-username=<user> --docker-password=<password> -n kagent`.
