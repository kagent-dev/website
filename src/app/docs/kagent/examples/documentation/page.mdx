...
Create a `toolserver.yaml` file:

```yaml
# Kagent ToolServer Configuration
# This connects agents to the MCP documentation query server
apiVersion: kagent.dev/v1alpha1
kind: ToolServer
metadata:
  name: sqlite-vec              # Name referenced by agents (in toolServer field)
  namespace: kagent             # Must match your Kagent installation namespace
spec:
  config:
    streamableHttp:             # Configuration for HTTP-based MCP connection
      sseReadTimeout: 5m0s      # Timeout for server-sent events
      timeout: 5s               # General connection timeout
      url: http://mcp-sqlite-vec.kagent:3001/mcp  # MCP server endpoint
    type: ''                    # Auto-detected based on config
  description: 'SQLite vector search MCP server for documentation queries'
```
...
kind: Agent
metadata:
  name: oswaldo                    # Agent name - change to your preference
  namespace: kagent               # Must match your Kagent installation namespace
spec:
  description: >-
    The MCP agent is answering questions about MCP, using the MCP documentation
  modelConfig: default-model-config  # References a ModelConfig resource for AI model settings
  systemMessage: >-
    Use your tool to answer any question about the Model Context Protocol (MCP).
    Use mcp for the product and latest for the version.
  tools:
    - mcpServer:
        toolNames:
          - query_documentation   # The specific tool from the MCP server to use
        toolServer: kagent/sqlite-vec  # References the ToolServer resource (namespace/name)
      type: McpServer             # Indicates this is an MCP-based tool
```
