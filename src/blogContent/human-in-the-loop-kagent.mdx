...
- **[A2A protocol](/docs/kagent/examples/a2a-agents)** for agent-to-agent communication
- **MCP servers** for extending agent capabilities with custom tools
- **Skills** for packaging reusable agent behaviors

If you want to try it yourself, the full step-by-step tutorial is available in the [Human-in-the-Loop example](/docs/kagent/examples/human-in-the-loop).
