...
Every agent-tool binding carries an isolation mode. kagent implements only `Shared`, which runs the bound agent inside its parent's Actor. The two agents share one sandbox, and the nesting adds no compute. The schema also accepts `Dedicated`, a mode that would give the bound agent an Actor of its own. kagent does not implement `Dedicated` yet, so a binding that sets it fails to compile and the pair never becomes ready. For details about both modes, see [Shared and Dedicated isolation]({{< link path="skills-and-mcp/about-tools#shared-and-dedicated-isolation" >}}).
