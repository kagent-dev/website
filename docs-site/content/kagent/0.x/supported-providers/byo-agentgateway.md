...
1. Install agentgateway in your cluster. For more information, see the [agentgateway documentation](https://agentgateway.dev/docs/kubernetes/latest/documentation/setup/). Add `--set agentgatewayModels.enabled=true` to the Helm command for the agentgateway control plane.
...
1. Create a `ModelConfig` resource. Choose the tab that matches your agentgateway authentication configuration.
   ...
   {{< reuse "kagent-docs/snippets/review-table.md" >}} For more information, see the [API docs]({{< link path="resources/api-ref#modelconfigspec" >}}).
   ...
   | `openAI.baseUrl` | The Kubernetes service address of your agentgateway Gateway, including the `/v1` path, set in `$AGENTGATEWAY_URL`. |
   {{% /tab %}}
...
Agentgateway is now added as a model endpoint in kagent. Next, you can [create or update an agent]({{< link path="getting-started/first-agent" >}}) to use this model.