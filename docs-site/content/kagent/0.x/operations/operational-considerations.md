...
On OpenShift, also set the HAProxy route timeout via `ui.openshiftRoute.annotations`. For more information, see [Expose the UI outside the cluster]({{< link path="observability/launch-ui#expose-the-ui-outside-the-cluster" >}}).
